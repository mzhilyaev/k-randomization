...
The combined set $\bbx + \bbz$ gives raise to a histogram $h \in \N^d$ received by applying the before discussed mechanism M (clear true records plus fake records).  Hence, the $Pr [ M_r(x) = g ]$ can be written as a sum over of probabilities over the domain of $h$:
...
\implies & R_n(S) \ge R_{n+1}(S+1) \ge R_n(S+1)
\end{align*}
 
 
Suppose $R_n$ is $(\epsilon, \delta)$-private. Since $R_n$ is monotonically decreasing with $S$ (lemma  \eqref{lem:rs1} ), there must exist two values $\alpha + \beta = \delta$, such that $R_n$ is upper bounded on the left at a particular limiting value $S_\alpha$ 
 \begin{align*}
 R_n(S_\alpha) \le e^\epsilon \text{ and } P(S \le S_\alpha) \le \alpha
\end{align*}
And it's low bounded on the right at a particular limiting value $S_\beta$
 \begin{align*}
 R_n(S_\beta) \ge \frac{1}{e^\epsilon} \text{ and } P(S \ge S_\beta) \le \beta
\end{align*}
